package Acme::Class::DBI::NoThreats::__::Base;
...
use base "Acme::Class::DBI::NoThreats::__::Base";

use vars qw($VERSION);
$VERSION = '0.96';

use Acme::Class::DBI::NoThreats::ColumnGrouper;
use Acme::Class::DBI::NoThreats::Query;
use Carp ();
use List::Util;
...
__PACKAGE__->iterator_class('Acme::Class::DBI::NoThreats::Iterator');
__PACKAGE__->__grouper(Acme::Class::DBI::NoThreats::ColumnGrouper->new());
...
__PACKAGE__->add_relationship_type(
	has_a      => "Acme::Class::DBI::NoThreats::Relationship::HasA",
	has_many   => "Acme::Class::DBI::NoThreats::Relationship::HasMany",
	might_have => "Acme::Class::DBI::NoThreats::Relationship::MightHave",
);
...
sub _set_columns {
	...
	$class->__grouper(Acme::Class::DBI::NoThreats::ColumnGrouper->clone($class->__grouper)
...
sub _make_method {
	...
	$class->_carp("Column '$name' in $class clashes with built-in method")
		if Acme::Class::DBI::NoThreats->can($name)
...
# for future updates, as other parts of Acme::Class::DBI::NoThreats depend on it.
sub _attribute_set {
	my $self = shift;
	my $vals = @_ == 1 ? shift: {@_};
...
sub delete {
	...
	undef %$self;
	bless $self, 'Acme::Class::DBI::NoThreats::Object::Has::Been::Deleted';
	return 1;
}
...
sub _deflated_column {
	...
	return $val->id if UNIVERSAL::isa($val => 'Acme::Class::DBI::NoThreats');
	return "$val";
}
...
sub _check_classes {    # may automatically call from CHECK block in future
	while (my ($load_class, $by_class) = each %required_classes) {
		next if $load_class->isa("Acme::Class::DBI::NoThreats");
		$by_class->_croak(
...
Acme::Class::DBI::NoThreats is a fork of L<Class::DBI> version 0.96.  It is
identical in every aspect, except for the package name, and that the current
maintainer agrees B<NOT TO>:

=over

=item *

Threaten to sue users who subscribe to the mailing list, or to call the police.
...
threaten to sue users who reconstruct wikis or other materials related to this
project.
...
threaten to sue users who think things that the maintainer finds disagreeable.

=back

Note that, other than that, B<THERE ARE NO FURTHER GUARANTEES>. Specifically,
there's no guarantee that the maintainer of the original, pre-fork
L<Class::DBI> will abide by these terms. There's no guarantee that 
Class::DBI plugins will work. And there's also no guarantee that this
forked version will be maintained at all! (It's C<ACME::>, after all.)
...
In short, Acme::Class::DBI::NoThreats aims to make it simple to introduce 'best
...
Acme::Class::DBI::NoThreats works on a simple one class/one table model.  It is your
responsibility to have your database tables already set up. Automating that
process is outside the scope of Acme::Class::DBI::NoThreats.
...
directly from Acme::Class::DBI::NoThreats.  This gives you a convenient point to
place system-wide overrides and enhancements to Acme::Class::DBI::NoThreats's behavior.
...
Acme::Class::DBI::NoThreats needs to know how to access the database.  It does this
...
Inform Acme::Class::DBI::NoThreats what table you are using for this class:
...
Acme::Class::DBI::NoThreats helps you along a bit to set up the database connection.
...
When Acme::Class::DBI::NoThreats constructs SQL, it aliases your table name to a name
...
Acme::Class::DBI::NoThreats will use the sequence to generate a primary key value when
objects are created without one.

*NOTE* This method does not work for Oracle. However, Acme::Class::DBI::NoThreats::Oracle
...
Acme::Class::DBI::NoThreats will Do The Right Thing.
...
your criteria, this will return a Acme::Class::DBI::NoThreats::Iterator instance, which
...
Currently Acme::Class::DBI::NoThreats does not offer any per-column mechanism here.
...
All errors that are generated, or caught and propagated, by Acme::Class::DBI::NoThreats
...
For exceptions that are caught and propagated by Acme::Class::DBI::NoThreats, $message
...
been thrown 'below' Acme::Class::DBI::NoThreats (using Exception::Acme::Class::DBI::NoThreats for example). 
...
Acme::Class::DBI::NoThreats inherits from Class::Accessor and thus provides individual
...
destroyed (falls out of scope or the program ends) then Acme::Class::DBI::NoThreats's
...
routines, you'll want to manipulate data in a Acme::Class::DBI::NoThreats object without
...
intereacting directly with the hash that makes up a Acme::Class::DBI::NoThreats object
(the exact implementation of which may change in a future release) you
should use Acme::Class::DBI::NoThreats's low-level accessors. These appear 'private' to
...
By default, Acme::Class::DBI::NoThreats uses simple hash references to store object
...
Acme::Class::DBI::NoThreats and its subclasses overload the perl builtin I<stringify>
and I<bool> operators. This is a significant convenience.

The perl builtin I<bool> operator is overloaded so that a Acme::Class::DBI::NoThreats
...
When a Acme::Class::DBI::NoThreats object reference is used in a string context it will,
...
fields:
	...
	package Widget;
	use base 'My::Acme::Class::DBI::NoThreats';
	Widget->table('widget');
...
Databases are all about relationships. And thus Acme::Class::DBI::NoThreats provides a
...
If the foreign class is another Acme::Class::DBI::NoThreats representation we will
...
and will be passed the value and the Acme::Class::DBI::NoThreats object as arguments.
...
it will be passed the Acme::Class::DBI::NoThreats object as an argument.
...
Acme::Class::DBI::NoThreats should be able to do the right thing). If no such has_a()
...
Acme::Class::DBI::NoThreats does not currently support Many to Many relationships, per se.
...
Acme::Class::DBI::NoThreats::Relationship subclasses.  These are then plugged into
Acme::Class::DBI::NoThreats through an add_relationship_type() call:
...
of the existing types.  See L<Acme::Class::DBI::NoThreats::Relationship> for more information
on what is required.
...
Acme::Class::DBI::NoThreats do the work for you, or just inline the entire query.
...
you can fall back on the fact that Acme::Class::DBI::NoThreats inherits from Ima::DBI
and prefers to use its style of dealing with statements, via set_sql().

The Acme::Class::DBI::NoThreats set_sql() method defaults to using prepare_cached()
...
=head2 Acme::Class::DBI::NoThreats::AbstractSearch
...
The L<Acme::Class::DBI::NoThreats::AbstractSearch> module, available from CPAN, is a
plugin for Acme::Class::DBI::NoThreats that allows you to write arbitrarily complex
...
$sth->select_val() call, coupled with Acme::Class::DBI::NoThreats's sql_single SQL.
...
In the tradition of Perl, Acme::Class::DBI::NoThreats is lazy about how it loads your
...
columns to the one group, and Acme::Class::DBI::NoThreats will load everything at once.
...
Acme::Class::DBI::NoThreats will put it there if you don't.
...
The columns of a class are stored as Acme::Class::DBI::NoThreats::Column objects. This
...
Acme::Class::DBI::NoThreats suffers from the usual problems when dealing with transactions.
...
Acme::Class::DBI::NoThreats supports uniqueness of objects in memory. In a given perl
...
Acme::Class::DBI::NoThreats will not maintain this lookup index, but give you a separate
...
You can call this method on any class or instance of Acme::Class::DBI::NoThreats, but
...
The preferred method of interacting with Acme::Class::DBI::NoThreats is for you to write
...
As the innards of Acme::Class::DBI::NoThreats are still in flux, you must exercise extreme
caution in overriding private methods of Acme::Class::DBI::NoThreats (those starting with
...
Theoretically Acme::Class::DBI::NoThreats should work with almost any standard RDBMS. Of
...
using them.
	...
	L<Acme::Class::DBI::NoThreats::mysql>, L<Acme::Class::DBI::NoThreats::Pg>, L<Acme::Class::DBI::NoThreats::Oracle>,
	L<Acme::Class::DBI::NoThreats::SQLite>
...
=head1 FORK MAINTAINER

Ivan Tubert-Brohman <itub@cpan.org>

=head1 Class::DBI AUTHOR
...
Uri Gutman, William McKee, the Acme::Class::DBI::NoThreats mailing list, the POOP group,
...
L<http://perlmonks.org/index.pl?node_id=479074> provides several pointers on 
the history before this fork.
...
of different approaches to database persistence, such as Acme::Class::DBI::NoThreats,
Alazabo, Tangram, SPOPS etc.

Acme::Class::DBI::NoThreats is built on top of L<Ima::DBI>, L<Class::Accessor> and
